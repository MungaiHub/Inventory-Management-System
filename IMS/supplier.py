...
class supplierclass:
    def __init__(self,root):
        ...
        txt_search=Entry(self.root,textvariable=self.var_searchtxt,font=("goudy old style",15),bg="lightyellow").place(x=800,y=80,width=162)
    ...
    def add(self):
        ...
        try:
            if self.var_sup_invoice.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Invice no. already assigned, try diffirent",parent=self.root)
                else:
