...
class productclass:
    def __init__(self,root):
        ...
        #=====buttons=========
        btn_save=Button(product_frame,text="Save",font=("goudy old style",15),bg="#2196f3",fg="white",cursor="hand2",command=self.add).place(x=10,y=400,width=100,height=40)
    ...
    def add(self):
        ...
        try:
            if self.var_cat.get()=="Select" or self.var_cat.get()=="Empty" or self.var_sup.get()=="Select" or self.var_name.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","product already present, try diffirent",parent=self.root)
                else:
